...
const MAX_TARGETS = 4; // лимит целевых модов
...
// связываем ключи с i18n-ключами
...
const expectedPicked = computed(() => {
  if (M.value === 0) return 0;
  const w = weightsNorm.value;
  const kAvg = 0*w[0] + 1*w[1] + 2*w[2] + 3*w[3] + 4*w[4];
  return kAvg * (tCount.value / M.value);
});
...
<template>
  <section class="mt-10">
    <div class="mx-auto max-w-5xl rounded-2xl border border-surface/40 bg-surface-light p-8 md:p-10">
      ...
      <p class="mt-3 text-text-muted/90">
        {{ t("calculator.subtitle") }}
      </p>

      <div class="mt-6 grid gap-4 sm:grid-cols-2">
        <!-- Веса -->
        <div class="rounded-2xl border border-surface/40 bg-surface p-5 shadow-sm transition hover:shadow-md hover:border-primary/30">
          ...
          <div class="mt-3 space-y-3">
            ...
            <div class="text-xs">
              <span class="text-text-muted">{{ t("calculator.weights.norm") }}:</span>
              <span class="ml-1 font-mono">
                [ {{ weightsNorm[0].toFixed(4) }}, {{ weightsNorm[1].toFixed(4) }}, {{ weightsNorm[2].toFixed(4) }}, {{ weightsNorm[3].toFixed(4) }}, {{ weightsNorm[4].toFixed(4) }} ]
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="mt-6 rounded-2xl border border-amber-300/40 bg-amber-300/10 p-5">
        ...
        <div class="mt-3 flex flex-wrap gap-2">
          ...
          >
            <span
                ...
                :class="state.picked.has(m) ? 'bg-amber-400' : 'bg-surface/60'"
            ></span>
            {{ modName(m) }}
          </button>
        </div>
        ...
        <p class="mt-2 text-xs text-text-muted">
          {{ reachedLimit
            ? t("calculator.targets.limit")
            : t("calculator.targets.hint")
          }}
        </p>
      </div>
      ...
      <div class="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div class="rounded-2xl border border-surface/40 bg-surface p-5 shadow-sm transition hover:shadow-md hover:border-primary/30">
          <h3 class="text-lg font-semibold text-primary">
            {{ t("calculator.results.expect") }}
          </h3>
          <div class="mt-3 flex items-center justify-between text-sm">
            <span class="text-text-muted">{{ t("calculator.results.expCount") }}</span>
            <strong class="font-mono text-text">{{ expectedPicked.toFixed(4) }}</strong>
          </div>
          <p class="mt-2 text-xs text-text-muted leading-relaxed">
            {{ t("calculator.results.formula") }}
          </p>
        </div>
      </div>
    </div>
